...
import           Lib                 (countLines, hideProgressBar, prettyPrint)
import           Options.Applicative (Parser, ParserInfo, argument, execParser,
                                      fullDesc, header, help, helper, info,
                                      long, metavar, progDesc, short, str,
                                      switch, (<**>))
...
parseOptions =
  Options
    ...
    <*> switch
          (long "git" <> help "Clone a Git repository from the provided URL")
    <*> argument
          str
          (metavar "PATH"
             <> help
                  "Path to analyze or URL of the repository if --git is specified")
...
main = do
  ...
  if optGit options
    then withSystemTempDirectory "repo-clone" $ \tempDir -> do
           when verbose $ putStrLn $ "Cloning repository: " ++ optPath options
             ...
             then callProcess "git" ["clone", optPath options, tempDir]
               ...
               _ <-
                 readCreateProcess
                   ...
                   ""
               return ()
           when verbose $ putStrLn $ "Repository cloned to: " ++ tempDir
      ...
      when verbose $ putStrLn $ "Analyzing path: " ++ optPath options
